 ...
 * 
 * This application uses a "Role-Based SPA" architecture.
 * 
 * Routing Strategy:
 * Instead of using a heavy router library (like React Router) for this stage,
 * we use a lightweight state-based router controlled by `currentPath`.
 * 
 * - Navigation: Components trigger a custom window event 'app-navigate' to change routes.
 * - Dynamic Routes: We handle dynamic paths (e.g., /admin/orders/123) by checking
 *   string prefixes in the `renderContent` function.
 * 
 * Security:
 * - The `userRole` state determines whether the ADMIN or CLIENT sidebar/routes are rendered.
 * - This effectively creates two separate apps within one codebase.
 */
...
function AppContent() {
  ...
  const handleLogin = (role: UserRole) => {
    setUserRole(role);
    setCurrentPath(role === UserRole.ADMIN ? '/admin/dashboard' : '/client/dashboard');
  };
  ...
  const renderContent = () => {
    if (userRole === UserRole.ADMIN) {
        ...
        if (currentPath.startsWith('/admin/users/')) {
            ...
            return <UserDetails userId={userId || ''} onBack={() => handleNavigate('/admin/users')} />;
        }
        if (currentPath.startsWith('/admin/orders/')) {
            const orderId = currentPath.split('/').pop();
            return <OrderDetails orderId={orderId || ''} onBack={() => handleNavigate('/admin/orders')} />;
        }
        if (currentPath.startsWith('/admin/invoices/')) {
            const invoiceId = currentPath.split('/').pop();
            return <InvoiceDetails invoiceId={invoiceId || ''} onBack={() => handleNavigate('/admin/invoices')} />;
        }
        if (currentPath.startsWith('/admin/freight/')) {
            const freightId = currentPath.split('/').pop();
            return <FreightDetails freightId={freightId || ''} onBack={() => handleNavigate('/admin/freight')} />;
        }
        if (currentPath.startsWith('/admin/payments/')) {
            const paymentId = currentPath.split('/').pop();
            return <PaymentDetails paymentId={paymentId || ''} onBack={() => handleNavigate('/admin/payments')} />;
        }
        if (currentPath.startsWith('/admin/shopping/')) {
            const requestId = currentPath.split('/').pop();
            return <ShoppingDetails requestId={requestId || ''} onBack={() => handleNavigate('/admin/shopping')} />;
        }
        if (currentPath.startsWith('/admin/inventory/')) {
            const id = currentPath.split('/').pop();
            return <InventoryDetails id={id || ''} onBack={() => handleNavigate('/admin/inventory')} />;
        }
        if (currentPath.startsWith('/admin/expenses/')) {
            const id = currentPath.split('/').pop();
            return <ExpenseDetails id={id || ''} onBack={() => handleNavigate('/admin/expenses')} />;
        }
        if (currentPath.startsWith('/admin/suppliers/')) {
            const id = currentPath.split('/').pop();
            return <SupplierDetails id={id || ''} onBack={() => handleNavigate('/admin/suppliers')} />;
        }
        if (currentPath.startsWith('/admin/delivery/')) {
            const id = currentPath.split('/').pop();
            return <DeliveryDetails id={id || ''} onBack={() => handleNavigate('/admin/delivery')} />;
        }
        if (currentPath.startsWith('/admin/tickets/')) {
            const id = currentPath.split('/').pop();
            return <TicketDetails id={id || ''} onBack={() => handleNavigate('/admin/tickets')} />;
        }
      ...
      if (currentPath.startsWith('/client/orders/')) {
          const id = currentPath.split('/').pop();
          return <ClientOrderDetails id={id || ''} onBack={() => handleNavigate('/client/orders')} />;
      }
      if (currentPath.startsWith('/client/shopping/')) {
          const id = currentPath.split('/').pop();
          return <ClientShoppingDetails id={id || ''} onBack={() => handleNavigate('/client/shopping')} />;
      }
      if (currentPath.startsWith('/client/invoices/')) {
          const id = currentPath.split('/').pop();
          return <ClientInvoiceDetails id={id || ''} onBack={() => handleNavigate('/client/invoices')} />;
      }
      if (currentPath.startsWith('/client/support/')) {
          const id = currentPath.split('/').pop();
          return <ClientTicketDetails id={id || ''} onBack={() => handleNavigate('/client/support')} />;
      }
