...
const Inventory: React.FC = () => {
    ...
    setItems((prev) =>
      prev.map((i) =>
        i.id === selectedItem?.id
          ? { ...i, location: newBin, status: "STORED" }
          : i
      )
    );
  ...
  const handleReportDamage = (e: React.FormEvent) => {
    ...
    setItems((prev) =>
      prev.map((i) =>
        i.id === selectedItem?.id ? { ...i, status: "DAMAGED" } : i
      )
    );
  ...
  const handleBulkPrint = () => {
    showToast(
      `Sent ${selectedIds.length} labels to thermal printer queue`,
      "success"
    );
  ...
  const handleBulkDelete = () => {
    if (
      confirm(
        `Are you sure you want to delete ${selectedIds.length} items from inventory?`
      )
    ) {
  ...
  const handleBulkMove = () => {
    ...
    setItems((prev) =>
      prev.map((i) =>
        selectedIds.includes(i.id) ? { ...i, location: "BULK-STAGING" } : i
      )
    );
  ...
  const BarcodeGenerator = ({
    ...
    for (let i = 0; i < value.length; i++)
      pattern += getBarPattern(value[i]) + "0";
    pattern += sentinel;
    ...
    return (
      <div className="flex flex-col items-center w-full">
        <svg
          height={height}
          width="100%"
          viewBox={`0 0 ${pattern.length * 2} ${height}`}
        ...
        <div className="font-mono font-bold tracking-[0.3em] text-sm mt-1 uppercase">
          {value}
        </div>
      </div>
    );
  ...
  const columns: Column<InventoryItem>[] = [
    {
      header: "HWB ID",
      accessor: (item) => (
        <span className="font-mono font-medium text-primary-700 hover:underline">
          {item.id}
        </span>
      ),
      sortKey: "id",
    ...
    },
    {
      ...
      accessor: (item) => (
        <div className="flex justify-end space-x-2">
          <button
            onClick={(e) => handleOpenBin(e, item)}
            className="p-1.5 text-slate-500 hover:text-blue-600 rounded bg-slate-50 hover:bg-blue-50"
          ...
          <button
            ...
            className="p-1.5 text-slate-500 hover:text-slate-800 rounded bg-slate-50 hover:bg-slate-200"
            title="Print Barcode"
          >
            <Printer size={16} />
          </button>
          <button
            onClick={(e) => handleOpenDamage(e, item)}
            className="p-1.5 text-slate-500 hover:text-red-600 rounded bg-slate-50 hover:bg-red-50"
            title="Report Damage"
          >
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-slate-500 text-sm">
            Manage rack locations, barcodes, and cargo conditions.
          </p>
        </div>
      </div>
      ...
      {selectedIds.length > 0 && (
        <div className="bg-slate-800 text-white p-3 rounded-lg shadow-lg flex items-center justify-between animate-in slide-in-from-top-2 fade-in">
          <div className="flex items-center">
            <span className="bg-white text-slate-900 px-2 py-0.5 rounded text-xs font-bold mr-3">
          ...
          <div className="flex space-x-2">
            ...
            <button
              ...
              className="flex items-center px-3 py-1.5 bg-blue-600 hover:bg-blue-500 rounded text-sm transition"
            >
              <Grid size={14} className="mr-2" /> Move to Bulk
            </button>
            <button
              onClick={handleBulkDelete}
              className="flex items-center px-3 py-1.5 bg-red-600 hover:bg-red-500 rounded text-sm transition"
            >
      ...
      <Modal
        isOpen={modalType === "BIN"}
        onClose={() => setModalType(null)}
        title={`Assign Location: ${selectedItem?.id}`}
      >
      ...
      {/* DAMAGE REPORT MODAL */}
      <Modal
        isOpen={modalType === "DAMAGE"}
        onClose={() => setModalType(null)}
        title={`Report Damage: ${selectedItem?.id}`}
      >
        <form onSubmit={handleReportDamage} className="space-y-4">
          <div className="bg-red-50 p-4 rounded border border-red-100 text-red-800 text-sm">
            Reporting damage will trigger an insurance workflow and notify the
            client immediately.
          </div>
          <div>
            <label className="block text-sm font-medium text-slate-700">
            ...
            <textarea
              ...
              className="w-full border p-2 rounded mt-1 bg-white text-slate-900"
              rows={3}
              placeholder="e.g. Wet carton, crushed corner..."
            ></textarea>
          </div>
          <div>
            <label className="block text-sm font-medium text-slate-700">
            ...
            <div className="flex justify-center mb-4">
              <div className="flex items-center bg-slate-100 rounded p-1">
                <button
                  ...
                  className="px-3 py-1 hover:bg-white rounded font-bold text-slate-600"
                >
                  -
                </button>
                <span className="px-4 font-mono text-sm font-bold">
                  {labelCopies} Copy
                </span>
                <button
            ...
            <div className="border-4 border-slate-800 bg-white p-4 mx-auto w-[300px] h-[450px] flex flex-col justify-between shadow-xl relative print:shadow-none print:border-2 print:border-black">
              ...
              <div className="text-center border-b-2 border-black pb-2">
                ...
                <p className="text-xs font-bold uppercase">
                  Origin:{" "}
                  {selectedItem.warehouse === "CN"
                    ? "GUANGZHOU, CN"
                    : selectedItem.warehouse === "US"
                    ? "NEW YORK, USA"
                    : "LONDON, UK"}
                </p>
              </div>
            ...
            <div className="flex justify-center gap-3 pt-4 border-t">
              ...
              <button
                onClick={() => {
                  ...
                  showToast(
                    `Sent ${labelCopies} label(s) to thermal printer`,
                    "success"
                  );
                  window.print();
